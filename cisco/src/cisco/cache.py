...
carries its own TTL, enabling per-data-type expiration suited to Cisco SG-300
response freshness requirements:

    - VLANs:              5 minutes (300 s) -- VLAN config changes rarely
    - Interfaces:         2 minutes (120 s) -- port status changes moderately
    - MAC table:          30 seconds (30 s) -- dynamic, changes frequently
    - LLDP neighbors:     5 minutes (300 s) -- topology is relatively stable
    - Running config:     10 minutes (600 s) -- full config dump, expensive
    - System info:        10 minutes (600 s) -- firmware/model/uptime
    - Interface counters: 1 minute  (60 s)  -- traffic stats change continuously
    - Spanning tree:      2 minutes (120 s)  -- STP topology changes are rare
...
Typical usage with the Cisco SSH client::

    cache = TTLCache(max_size=500, default_ttl=120.0)

    vlans = await cache.get_or_fetch(
        "vlans:all",
        fetcher=lambda: ssh_client.show_vlans(),
        ttl=CacheTTL.VLANS,
    )
...
# ---------------------------------------------------------------------------
# Default TTLs for Cisco SG-300 data types
# ---------------------------------------------------------------------------


class CacheTTL:
    """Default TTL values (in seconds) for Cisco SG-300 data types.

    These values reflect how frequently each data type changes in typical
    managed switch deployments. Use these constants when calling ``get_or_fetch``
    to ensure consistent caching behaviour across the plugin.
    """

    VLANS: float = 300.0  # 5 minutes -- VLAN config changes rarely
    INTERFACES: float = 120.0  # 2 minutes -- port status changes moderately
    MAC_TABLE: float = 30.0  # 30 seconds -- dynamic, changes frequently
    LLDP_NEIGHBORS: float = 300.0  # 5 minutes -- topology is relatively stable
    RUNNING_CONFIG: float = 600.0  # 10 minutes -- full config dump, expensive
    SYSTEM_INFO: float = 600.0  # 10 minutes -- firmware/model/uptime
    INTERFACE_COUNTERS: float = 60.0  # 1 minute -- traffic stats change continuously
    SPANNING_TREE: float = 120.0  # 2 minutes -- STP topology changes are rare
...
class TTLCache:
    ...
    async def flush_by_prefix(self, prefix: str) -> None:
        ...
        ``await cache.flush_by_prefix("vlans:")`` after a VLAN update.
