...
Covers Talos-specific default TTLs (CacheTTL) and the full cache
...
from talos.cache import CacheTTL, TTLCache
...
class TestCacheTTL:
    """Verify Talos-specific default TTL constants."""

    def test_node_list_ttl(self) -> None:
        assert CacheTTL.NODE_LIST == 300.0  # 5 minutes

    def test_cluster_health_ttl(self) -> None:
        assert CacheTTL.CLUSTER_HEALTH == 60.0  # 1 minute

    def test_etcd_status_ttl(self) -> None:
        assert CacheTTL.ETCD_STATUS == 30.0  # 30 seconds

    def test_service_status_ttl(self) -> None:
        assert CacheTTL.SERVICE_STATUS == 60.0  # 1 minute

    def test_resource_query_ttl(self) -> None:
        assert CacheTTL.RESOURCE_QUERY == 120.0  # 2 minutes

    def test_version_info_ttl(self) -> None:
        assert CacheTTL.VERSION_INFO == 600.0  # 10 minutes

    def test_config_generation_ttl(self) -> None:
        assert CacheTTL.CONFIG_GENERATION == 0.0  # never cache
...
class TestGetOrFetch:
    ...
    async def test_fetcher_called_on_miss(self) -> None:
        cache = TTLCache()
        fetcher = AsyncMock(return_value={"nodes": ["cp-1", "cp-2", "w-1"]})

        result = await cache.get_or_fetch("key", fetcher)

        assert result == {"nodes": ["cp-1", "cp-2", "w-1"]}
    ...
    async def test_custom_ttl_on_get_or_fetch(self) -> None:
        ...
        await cache.get_or_fetch("key", fetcher, ttl=CacheTTL.ETCD_STATUS)
    ...
    async def test_fetcher_exception_propagates(self) -> None:
        ...
        with pytest.raises(ConnectionError, match="node unreachable"):
...
class TestFlush:
    ...
    async def test_flush_by_prefix(self) -> None:
        cache = TTLCache()
        await cache.set("nodes:all", ["cp-1", "cp-2"])
        await cache.set("nodes:cp-1", {"hostname": "talos-cp-1"})
        await cache.set("health:cluster", {"healthy": True})

        await cache.flush_by_prefix("nodes:")

        assert await cache.get("nodes:all") is None
        assert await cache.get("nodes:cp-1") is None
        assert await cache.get("health:cluster") == {"healthy": True}

    @pytest.mark.asyncio
    async def test_flush_nonexistent_key(self) -> None:
        """Flushing a key that doesn't exist should not error."""
        cache = TTLCache()
...
class TestStats:
    ...
    async def test_stats_after_operations(self) -> None:
        ...
        await cache.set("c", 3)  # evicts "b" (since "a" was promoted by get)
