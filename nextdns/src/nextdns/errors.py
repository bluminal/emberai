...
"""Structured error hierarchy for the nextdns plugin.
...
class NetexError(Exception):
    """Base exception for all Netex / nextdns plugin errors.
    ...
    endpoint:
        API endpoint path (e.g. ``/profiles/abcdef``).
    retry_hint:
...
class WriteGateError(NetexError):
    ...
    The safety gate (see ``nextdns/safety.py``) enforces two preconditions
    ...
    _REASON_MESSAGES: ClassVar[dict[WriteGateReason, str]] = {
        WriteGateReason.ENV_VAR_DISABLED: (
            "Set NEXTDNS_WRITE_ENABLED=true to allow write operations"
        ),
