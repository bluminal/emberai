...
carries its own TTL, enabling per-data-type expiration tuned to NextDNS data
volatility:

- Profile list:    5 min (300s)  — profiles change infrequently
- Single profile:  2 min (120s)  — settings may be actively edited
- Analytics:      30 sec  (30s)  — fast-changing query/block counts
- Logs:           no cache       — always fetch fresh (real-time tailing)

Typical usage with the NextDNS API client::

    cache = TTLCache(max_size=500, default_ttl=300.0)

    profiles = await cache.get_or_fetch(
        "profiles:all",
        fetcher=lambda: api.list_profiles(),
        ttl=300.0,
    )

    analytics = await cache.get_or_fetch(
        "analytics:profile-abc",
        fetcher=lambda: api.get_analytics("abc"),
        ttl=30.0,
    )

The cache is not persistent across process restarts. A Redis-backed
implementation is planned for Phase 5 (Milestone 5.1).
"""
...
class TTLCache:
    ...
    def __init__(self, max_size: int = 1000, default_ttl: float = 300.0) -> None:
        ...
        # In-flight fetches for stampede protection. Maps a cache key to an
        # Event + result container so concurrent callers can await the first
        # fetch instead of issuing duplicates.
    ...
    async def flush_by_prefix(self, prefix: str) -> None:
        ...
        ``await cache.flush_by_prefix("profiles:")`` after a profile update.
