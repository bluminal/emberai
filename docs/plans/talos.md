...
| # | Task | Complexity | Dependencies | Status |
|---|------|-----------|--------------|--------|
| 40 | Create `docs/talos/overview.md`: plugin purpose, architecture (CLI wrapper, not REST), Talos Linux primer (immutable OS, API-driven, no SSH), supported operations, required ports. | M | None | done |
| 41 | Create `docs/talos/commands.md`: reference docs for Phase 1 commands (`cluster-setup`, `cluster-status`) with usage examples. | M | M1.5 | done |
| 42 | Create `docs/talos/skills.md`: reference docs for Phase 1 skills (`config`, `cluster`) with all tool signatures. | M | M1.4 | done |
| 43 | Update `docs/getting-started/authentication.md`: add Talos talosconfig setup (how to generate, where to store, env var configuration) alongside existing UniFi, OPNsense, NextDNS, and Cisco auth docs. | S | None | done |
| 44 | Write 3 workflow examples: (a) first-time cluster setup (the full 10-step guided process), (b) check cluster health after setup, (c) generate and validate config patches. Follow 7-section template. Include "Working Safely" section emphasizing bootstrap irreversibility and disk wipe warnings. | L | M1.5 | done |
| 45 | Create EmberAI marketplace packaging: ensure `talos/pyproject.toml` has correct metadata, entry points, and README for marketplace listing. | S | M1.5 | done |
