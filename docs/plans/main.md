...
Netex is a four-plugin network intelligence suite for the EmberAI marketplace: **unifi** (edge layer), **opnsense** (gateway layer), **nextdns** (DNS layer), and **netex** (cross-vendor umbrella orchestrator). This plan implements the full PRD (`docs/reqs/main.md`) across 6 plan phases. Phase 1 goes deep on core unifi skills via the Local Gateway API. Phase 2 completes unifi and scaffolds opnsense. Phase 3 completes opnsense. Phase 3.5 builds the nextdns plugin. Phase 4 builds the netex umbrella. Phase 5 adds advanced features.
...
| Plan Phase 3.5 (v0.2.5-plan) | New — DNS layer | nextdns standalone plugin complete |
| Plan Phase 4 (v0.3.0-plan) | PRD Phase 3 | netex umbrella complete (incl. nextdns cross-vendor integration) |
...
| D16 | NextDNS plugin as fourth vendor plugin with `dns` role | DNS filtering and analytics are a distinct network layer not covered by OPNsense Unbound or UniFi. NextDNS provides per-profile policy enforcement, analytics, and parental controls. | NextDNS operates at the DNS resolution layer above the gateway. The `dns` role is new to the Vendor Plugin Contract -- distinct from the `services` skill group's DNS tools (which manage local resolver config). |
| D17 | NextDNS API key via `X-Api-Key` header, single credential | NextDNS API uses a single API key from the account dashboard. No per-profile auth. | Simple auth model. One env var (`NEXTDNS_API_KEY`). No OAuth, no per-resource credentials. |
| D18 | NextDNS write operations limited to profile config and deny/allow lists | The NextDNS API supports profile CRUD, sub-resource updates, and log deletion. Profile deletion and log clearing are destructive. | Write gate (D8) applies to all mutations. Profile deletion requires explicit `--delete-profile` flag in addition to `--apply` to prevent accidental destruction. Log clearing gated separately. |
| D19 | NextDNS plugin built in Phase 3.5, cross-vendor integration in Phase 4 | Standalone plugin (read/write tools, analytics) has no dependency on the netex umbrella. Cross-vendor features (VLAN-to-profile verification, DNS path tracing) depend on the orchestrator. | Building the standalone plugin early proves out the API client and skill surface. Cross-vendor tasks are sequenced into Phase 4 where the umbrella already exists. |
| D20 | Cursor-based pagination for NextDNS analytics and logs | NextDNS API uses cursor-based pagination (not offset-based). List responses include a `cursor` field for the next page. | API client must implement cursor iteration. Analytics tools should support `--limit` for bounded queries and `--all` for full iteration with backpressure. Default limit: 100 items. |
...
| Q11 | NextDNS API rate limits -- the API documentation does not publish rate limits. Need to discover empirically or contact NextDNS support. | Affects analytics batch queries and real-time log streaming. If limits are tight, need client-side throttling. | Open -- discover during Phase 3.5 M3.5.2 API client implementation |
| Q12 | NextDNS device-to-profile assignment -- devices are identified passively via DNS client naming (DoH/DoT), not assigned via API. How do we correlate NextDNS device IDs with UniFi client MACs? | Affects cross-vendor device tracing in Phase 4. May need heuristic matching (IP address, hostname, timing). | Open -- resolve in Phase 4 cross-vendor integration |
| Q13 | NextDNS SSE log streaming -- does the `mcp` SDK support long-lived SSE connections for real-time log forwarding? | Affects `nextdns__logs__stream` tool design. May need to buffer and flush rather than true streaming. | Open -- resolve during Phase 3.5 M3.5.4 |
| Q14 | VLAN-to-profile pinning verification -- how to reliably confirm that DNS queries from a specific VLAN are hitting the correct NextDNS profile? | Analytics API shows source IPs but not VLAN tags. Must correlate OPNsense VLAN subnet ranges with NextDNS source IPs. | Open -- resolve in Phase 4 cross-vendor integration |
...
## Plan Phase 3.5: nextdns Plugin -- DNS Layer Intelligence (v0.2.5-plan)

### Milestone 3.5.1: Project Scaffold + CI + Foundations

| # | Task | Complexity | Dependencies | Status |
|---|------|-----------|--------------|--------|
| 194 | Create `nextdns/pyproject.toml` with dependencies: `mcp`, `httpx`, `pydantic`, `python-dotenv`. Dev deps: `pytest`, `pytest-asyncio`, `coverage`, `ruff`, `mypy`. Define `[project.scripts]` entry point for MCP server. Add `[project.entry-points."netex.plugins"]` entry point for Plugin Registry discovery (D12). | M | None | done |
| 195 | Create directory structure: `nextdns/src/{agents,api,tools,models}/` with `__init__.py` files. Create `nextdns/tests/` with `conftest.py`. Create `.env.example` with `NEXTDNS_API_KEY` (required) and `NEXTDNS_WRITE_ENABLED` (optional, default false). | S | None | done |
| 196 | Create `nextdns/SKILL.md`: YAML frontmatter (`netex_vendor: nextdns`, `netex_role: [dns]`, `netex_skills: [profiles, analytics, logs, security-posture]`, `netex_write_flag: NEXTDNS_WRITE_ENABLED`, `netex_contract_version: 1.0.0`). Overview, auth (API key via `X-Api-Key` header, D17), interaction model sections. | M | None | done |
| 197 | Create MCP server entry point (`nextdns/src/server.py`): initialize `mcp.Server`, configure transport via `--transport stdio|http` (D13), env var loading and validation (`NEXTDNS_API_KEY` required, `NEXTDNS_WRITE_ENABLED` optional). Add `--check` flag for startup health probe (verify API key set, attempt `GET /profiles` to confirm connectivity, report status, exit). Structured logging. | M | Task 194 | done |
| 198 | Create GitHub Actions CI workflow (`.github/workflows/nextdns.yml`): ruff lint, mypy type-check, pytest with coverage (80% threshold). Trigger on PR changes to `nextdns/`. | M | Task 194 | done |
| 199 | Create `nextdns/src/errors.py`: error hierarchy per D14 (`NetexError -> AuthenticationError, RateLimitError, NetworkError, APIError, ValidationError, WriteGateError`). All errors carry structured context (status code, endpoint, retry hint). | M | None | done |
| 200 | Create `nextdns/src/cache.py`: TTL cache (same design as unifi/opnsense -- D9). Profile list: 5 min, analytics: 30 sec (fast-changing), logs: no cache. | M | None | done |
| 201 | Create `nextdns/src/safety.py`: write gate decorator -- checks `NEXTDNS_WRITE_ENABLED` env var, validates `--apply` flag. Profile deletion requires additional `--delete-profile` flag (D18). Log clearing requires `--clear-logs` flag. | M | None | done |
| 202 | Create `nextdns/src/output.py`: OX output formatting for DNS-specific data -- profile summary tables, analytics charts (top domains, block reasons), device activity, security posture reports, deny/allow list views. | M | None | done |
| 203 | Create base Pydantic models in `nextdns/src/models/`: `Profile`, `SecuritySettings`, `PrivacySettings`, `ParentalControlSettings`, `DenylistEntry`, `AllowlistEntry`, `AnalyticsStatus`, `AnalyticsDomain`, `AnalyticsDevice`, `LogEntry`, `ProfileSettings`. Use strict mode, field aliases for API response normalization. | M | None | done |
| 204 | Create `nextdns/tests/fixtures/`: realistic mock NextDNS API response JSON files for profiles (with all nested settings), analytics endpoints, log entries. Include edge cases: empty profiles, profiles with extensive blocklists, analytics with zero queries. | M | None | done |

**Parallelizable:** Tasks 194-196, 199-204 can all run concurrently (10 tasks). Tasks 197-198 depend on 194. Max 10 concurrent.
**Milestone Value:** Runnable MCP server skeleton with CI pipeline, structured errors, caching, write gate (with profile deletion and log clearing safeguards), output formatting, and Pydantic models. `python -m nextdns.src.server` starts without errors.

### Milestone 3.5.2: NextDNS API Client

| # | Task | Complexity | Dependencies | Status |
|---|------|-----------|--------------|--------|
| 205 | Create `nextdns/src/api/nextdns_client.py`: async httpx client, base URL `https://api.nextdns.io`, `X-Api-Key` auth header from `NEXTDNS_API_KEY` (D17), request/response logging via structlog, error handling using error hierarchy (D14) -- map 401->AuthenticationError, 429->RateLimitError, 5xx->APIError, timeout->NetworkError. **Rate limit handling (Q11):** implement conservative initial throttling (100ms between requests), track response times, back off on 429 with exponential backoff (initial 1s, max 60s, jitter). Log any 429 occurrences to help discover actual limits. | L | M3.5.1 | done |
| 206 | Implement cursor-based pagination (D20): generic async iterator that follows `cursor` field in list responses. Support `limit` parameter for bounded queries. Default page size: 100 (API max: 500). Handle empty pages and missing cursor (end of results). | M | Task 205 | done |
| 207 | Implement sub-resource URL builder: given a profile ID and a dotted path (e.g., `security`, `privacy.blocklists`, `parentalControl.services`), construct the correct REST URL (`/profiles/{id}/security`, `/profiles/{id}/privacy/blocklists`, etc.). Handle both object endpoints (GET/PATCH) and array endpoints (GET/PUT/POST + child PATCH/DELETE). | M | Task 205 | done |
| 208 | Integrate TTL cache into API client: cache GET responses per D9 TTLs (profile list: 5 min, single profile: 2 min, analytics: 30 sec, logs: no cache). Post-write cache flush for affected profile and sub-resource keys. | M | Tasks 200, 205 | done |
| 209 | Write comprehensive tests for NextDNS API client: mock httpx responses, test API key header injection, cursor pagination (multi-page, single page, empty), sub-resource URL construction, cache hit/miss/expiry/post-write-flush, error handling for all status codes, rate limit backoff behavior. Use fixtures from Task 204. | L | Tasks 205-208 | done |

**Parallelizable:** Task 205 first. Tasks 206, 207, 208 can run concurrently after 205. Task 209 after all.
**Milestone Value:** Verified API client that can authenticate, paginate, build sub-resource URLs, and cache results. All tested with mocks.

### Milestone 3.5.3: Profiles Skill + Security Posture

| # | Task | Complexity | Dependencies | Status |
|---|------|-----------|--------------|--------|
| 210 | Implement `nextdns__profiles__list_profiles()` tool: call `GET /profiles`, normalize to `Profile` model list with summary (name, ID, key security/privacy settings enabled count). Register via `@server.tool()`. | M | M3.5.2 | done |
| 211 | Implement `nextdns__profiles__get_profile(profile_id)` tool: call `GET /profiles/{id}`, return full `Profile` model with all nested settings (security, privacy, parental control, settings). | M | M3.5.2 | done |
| 212 | Implement `nextdns__profiles__get_security(profile_id)` tool: call `GET /profiles/{id}/security`, return `SecuritySettings` model (threat intel, AI detection, Safe Browsing, cryptojacking, DNS rebinding, IDN homographs, typosquatting, DGA, NRD, DDNS, parking, CSAM, blocked TLDs). | S | M3.5.2 | done |
| 213 | Implement `nextdns__profiles__get_privacy(profile_id)` tool: call `GET /profiles/{id}/privacy`, return `PrivacySettings` model (blocklists with IDs and active status, native trackers, disguised trackers, affiliate links). | S | M3.5.2 | done |
| 214 | Implement `nextdns__profiles__get_parental_control(profile_id)` tool: call `GET /profiles/{id}/parentalControl`, return `ParentalControlSettings` model (blocked services, blocked categories, SafeSearch, YouTube Restricted Mode, bypass blocking status). | S | M3.5.2 | done |
| 215 | Implement `nextdns__profiles__get_denylist(profile_id)` and `nextdns__profiles__get_allowlist(profile_id)` tools: call respective endpoints, return list of entries with domain and active flag. | S | M3.5.2 | done |
| 216 | Implement `nextdns__profiles__get_settings(profile_id)` tool: call `GET /profiles/{id}/settings`, return logging config (enabled, retention, location), block page, performance settings (ECS, cache boost, CNAME flattening), Web3 toggle. | S | M3.5.2 | done |
| 217 | Implement `nextdns__security_posture__audit(profile_id?)` tool: when profile_id given, audit one profile. When omitted, audit all profiles. Check: (a) all security toggles enabled, (b) recommended blocklist active in privacy, (c) no overly broad allowlist entries, (d) logging enabled, (e) DNSSEC not disabled. Return severity-tiered findings per profile. | M | Tasks 210-216 | done |
| 218 | Implement `nextdns__security_posture__compare(profile_id_a, profile_id_b)` tool: diff two profiles across all settings sections. Return structured diff (setting name, profile A value, profile B value, recommendation). Highlight security regressions. | M | Tasks 210-216 | done |
| 219 | Create profiles agent (`nextdns/src/agents/profiles.py`): orchestrates profile tools to produce summary views. Lists all profiles with key security indicators. Single-profile detail view with all settings. Uses OX formatters. | M | Tasks 210-216 | done |
| 220 | Create security-posture agent (`nextdns/src/agents/security_posture.py`): orchestrates audit and compare tools. Severity-tiered output (Critical/Warning/Informational). Cross-profile baseline comparison. | M | Tasks 217-218 | done |
| 221 | Write tests for all profiles and security-posture tools: mock API responses for profiles with various setting combinations (all-on, all-off, mixed), audit with clean vs. weak profiles, compare with identical vs. divergent profiles. | L | Tasks 210-220 | done |

**Parallelizable:** Tasks 210-216 can all run concurrently (7 tools). Tasks 217-219 depend on 210-216 and can run concurrently (3 tasks). Task 220 depends on 217-218. Task 221 after all.
**Milestone Value:** Full read-only profile management and security auditing. Operators can list profiles, inspect settings, audit security posture, and compare profiles.

### Milestone 3.5.4: Analytics + Logs Skills

| # | Task | Complexity | Dependencies | Status |
|---|------|-----------|--------------|--------|
| 222 | Implement `nextdns__analytics__get_status(profile_id, from?, to?, device?)` tool: call `/profiles/{id}/analytics/status`, return query counts by status (default/blocked/allowed). Support time range and device filters. | M | M3.5.2 | done |
| 223 | Implement `nextdns__analytics__get_top_domains(profile_id, status?, from?, to?, device?, limit?)` tool: call `/profiles/{id}/analytics/domains`, return top domains with counts. Filter by status (blocked/allowed/default). Paginate via cursor (D20). | M | M3.5.2 | done |
| 224 | Implement `nextdns__analytics__get_block_reasons(profile_id, from?, to?, device?)` tool: call `/profiles/{id}/analytics/reasons`, return block/allow reasons with counts (which blocklist or rule triggered). | M | M3.5.2 | done |
| 225 | Implement `nextdns__analytics__get_devices(profile_id, from?, to?)` tool: call `/profiles/{id}/analytics/devices`, return device list with ID, name, model, localIp, query counts. | M | M3.5.2 | done |
| 226 | Implement `nextdns__analytics__get_protocols(profile_id, from?, to?)` tool: call `/profiles/{id}/analytics/protocols`, return DNS protocol breakdown (DoH, DoT, UDP, TCP, etc.). Highlight unencrypted protocols. | S | M3.5.2 | done |
| 227 | Implement `nextdns__analytics__get_encryption(profile_id, from?, to?)` tool: call `/profiles/{id}/analytics/encryption`, return encrypted vs. unencrypted query counts. Flag unencrypted percentage > 10% as warning. | S | M3.5.2 | done |
| 228 | Implement `nextdns__analytics__get_destinations(profile_id, type, from?, to?)` tool: call `/profiles/{id}/analytics/destinations`, support `type=countries` and `type=gafam`. Return breakdown by destination. | S | M3.5.2 | done |
| 229 | Implement remaining analytics tools (4 tools): `get_ips` (source IPs with ISP, ASN, geo), `get_query_types` (DNS record type breakdown), `get_ip_versions` (IPv4 vs IPv6), `get_dnssec` (DNSSEC validation stats). All follow same pattern: profile ID, time range, device filter, paginated. | M | M3.5.2 | done |
| 230 | Implement `nextdns__logs__search(profile_id, domain?, status?, device?, from?, to?, limit?)` tool: call `GET /profiles/{id}/logs` with query parameters. Return list of `LogEntry` objects (timestamp, domain, status, reasons, device info, client IP, protocol, encryption). Cursor pagination (D20). | M | M3.5.2 | done |
| 231 | Implement `nextdns__logs__stream(profile_id)` tool: connect to `GET /profiles/{id}/logs/stream` SSE endpoint. Buffer incoming events and return batches (Q13). If SSE streaming not feasible in MCP context, fall back to polling `GET /profiles/{id}/logs` with short interval (5 sec) and `from` timestamp tracking. Document limitation. | L | M3.5.2 | done |
| 232 | Implement `nextdns__logs__download(profile_id, from?, to?)` tool: call `GET /profiles/{id}/logs/download`, return or save bulk log data. Warn operator about large downloads. | S | M3.5.2 | done |
| 233 | Create analytics agent (`nextdns/src/agents/analytics.py`): orchestrates analytics tools into a dashboard-style summary per profile. Top blocked domains, query volume trends, device activity, protocol/encryption audit. Uses OX formatters. | M | Tasks 222-229 | done |
| 234 | Create logs agent (`nextdns/src/agents/logs.py`): orchestrates log search and stream tools. Investigation mode: "what did device X query in the last hour?" Uses OX formatters for log table output. | M | Tasks 230-232 | done |
| 235 | Write tests for all analytics and logs tools: mock API responses for analytics with various time ranges, device filters, empty results. Test cursor pagination across multiple pages. Test log stream fallback behavior. Test log download warning for large date ranges. | L | Tasks 222-234 | done |

**Parallelizable:** Tasks 222-232 can all run concurrently (11 tools). Tasks 233-234 depend on their respective tools. Task 235 after all. Max 11 concurrent.
**Milestone Value:** Full DNS analytics and log access. Operators can query volume stats, investigate blocked domains, audit protocol encryption, monitor device activity, and search/stream logs for troubleshooting.

### Milestone 3.5.5: Write Operations

| # | Task | Complexity | Dependencies | Status |
|---|------|-----------|--------------|--------|
| 236 | Implement `nextdns__profiles__create_profile(name, security?, privacy?, parental_control?)` write tool: `POST /profiles` with optional initial settings. Returns new profile ID and DNS endpoint URL. Write-gated (D8). | M | M3.5.3 | done |
| 237 | Implement `nextdns__profiles__update_profile(profile_id, name?)` write tool: `PATCH /profiles/{id}`. Write-gated. | S | M3.5.3 | done |
| 238 | Implement `nextdns__profiles__delete_profile(profile_id)` write tool: `DELETE /profiles/{id}`. **Extra safeguard (D18):** requires `--delete-profile` flag in addition to `--apply`. Presents profile name, ID, and active device count (from analytics) before confirmation. | M | M3.5.3, M3.5.4 | done |
| 239 | Implement `nextdns__profiles__update_security(profile_id, **settings)` write tool: `PATCH /profiles/{id}/security`. Accept any combination of security toggle fields. Write-gated. | S | M3.5.3 | done |
| 240 | Implement `nextdns__profiles__update_privacy(profile_id, blocklists?, native_trackers?, disguised_trackers?, affiliate_links?)` write tool: `PATCH /profiles/{id}/privacy`. For blocklists: `PUT /profiles/{id}/privacy/blocklists` to replace all, or `POST` to add one. Write-gated. | M | M3.5.3 | done |
| 241 | Implement `nextdns__profiles__update_parental_control(profile_id, services?, categories?, safe_search?, youtube_restricted?)` write tool: update parental control sub-resources. Write-gated. | M | M3.5.3 | done |
| 242 | Implement `nextdns__profiles__add_denylist_entry(profile_id, domain)` and `nextdns__profiles__remove_denylist_entry(profile_id, entry_id)` write tools. Write-gated. | S | M3.5.3 | done |
| 243 | Implement `nextdns__profiles__add_allowlist_entry(profile_id, domain)` and `nextdns__profiles__remove_allowlist_entry(profile_id, entry_id)` write tools. Write-gated. | S | M3.5.3 | done |
| 244 | Implement `nextdns__profiles__update_settings(profile_id, logs?, block_page?, performance?, web3?)` write tool: update profile-level settings (logging config, block page, ECS, cache boost, CNAME flattening, Web3). Write-gated. | S | M3.5.3 | done |
| 245 | Implement `nextdns__logs__clear(profile_id)` write tool: `DELETE /profiles/{id}/logs`. **Extra safeguard (D18):** requires `--clear-logs` flag. Presents log retention period and approximate log count before confirmation. Write-gated. | S | M3.5.4 | done |
| 246 | Implement `nextdns__profiles__apply_template(profile_ids[], template)` bulk tool: apply a set of security/privacy settings across multiple profiles. Iterates profiles, computes diff per profile, presents unified change plan via OX formatters, single confirmation. Write-gated. | L | Tasks 236-244 | done |
| 247 | Write tests for all write operations: test write gate enforcement (env var disabled, --apply missing), test profile deletion extra safeguard (--delete-profile missing), test log clearing safeguard (--clear-logs missing), test apply_template with multiple profiles (some needing changes, some already matching). | L | Tasks 236-246 | done |

**Parallelizable:** Tasks 236-245 can all run concurrently (10 tools). Task 246 depends on 236-244. Task 247 after all.
**Milestone Value:** Full profile lifecycle management. Operators can create, configure, and delete profiles. Manage deny/allow lists. Apply security templates across multiple profiles. Clear logs with safeguards.

### Milestone 3.5.6: Commands + Docs + Marketplace

| # | Task | Complexity | Dependencies | Status |
|---|------|-----------|--------------|--------|
| 248 | Implement `nextdns profiles` command: list all profiles with key indicators (security score, blocklist count, active device count). `--detail {id}` for full profile view. Uses profiles agent. | M | M3.5.3 | done |
| 249 | Implement `nextdns analytics {profile_id}` command: dashboard-style summary (query volume, top blocked, device activity, protocol audit). `--from`/`--to` time range. `--device` filter. Uses analytics agent. | M | M3.5.4 | done |
| 250 | Implement `nextdns audit` command: security posture audit across all profiles. `--profile {id}` for single profile. `--compare {id_a} {id_b}` for diff. Uses security-posture agent. | M | M3.5.3, M3.5.4 | done |
| 251 | Implement `nextdns logs {profile_id}` command: search logs with `--domain`, `--status`, `--device`, `--from`/`--to` filters. `--stream` for real-time mode (Q13). `--download` for bulk export. Uses logs agent. | M | M3.5.4 | done |
| 252 | Implement `nextdns manage {profile_id}` command: interactive profile configuration. Sub-commands: `--add-deny {domain}`, `--add-allow {domain}`, `--enable-security {setting}`, `--template {name}`. All write-gated. | M | M3.5.5 | done |
| 253 | Create `docs/nextdns/overview.md`, `docs/nextdns/commands.md`, `docs/nextdns/skills.md` -- reference docs for all nextdns commands and skills. | M | Tasks 248-252 | done |
| 254 | Update `docs/getting-started/authentication.md` -- add NextDNS API key setup alongside existing UniFi and OPNsense auth docs. | S | None | done |
| 255 | Write 5 nextdns workflow examples: first-time profile scan, DNS analytics review, security posture audit, parental control setup, investigate blocked domain. Follow 7-section template. Include "Working Safely" section. | L | Tasks 248-252 | done |
| 256 | Create EmberAI marketplace packaging for nextdns plugin: ensure `nextdns/pyproject.toml` has correct metadata, entry points, and README for marketplace listing. | S | M3.5.5 | done |
| 257 | Write tests for all nextdns commands: test command -> agent -> tool -> API flow for profiles, analytics, audit, logs, manage. Per-command test files. | L | Tasks 248-252 | done |

**Parallelizable:** Tasks 248-251 can run concurrently (4 commands). Task 252 depends on M3.5.5. Tasks 253-255 depend on 248-252. Task 254 is independent. Task 256 depends on M3.5.5. Task 257 after 248-252. Max 5 concurrent.
**Milestone Value:** Complete standalone nextdns plugin with all commands, documentation, and marketplace packaging. Operators can manage NextDNS profiles, review analytics, audit security posture, and investigate DNS logs through natural conversation.
...
| # | Task | Complexity | Dependencies | Status |
|---|------|-----------|--------------|--------|
| 115 | Create `netex/pyproject.toml`, directory structure (`src/{agents,registry,models,workflows,tools}/`), `netex/tests/`, `netex/SKILL.md` from PRD Appendix C. Add entry point. `.env.example`. | M | None | done |
| 116 | Create netex MCP server entry point with `--transport stdio|http` (D13), `--check` health probe, env var loading (`NETEX_WRITE_ENABLED`, `NETEX_CACHE_TTL`). Structured logging. | M | Task 115 | done |
| 117 | Implement abstract data model (`netex/src/models/abstract.py`): `VLAN`, `FirewallPolicy`, `Route`, `VPNTunnel`, `DNSRecord`, `DHCPLease`, `NetworkTopology`. Each with `from_vendor(vendor_name, raw_data)` class method and `to_vendor(vendor_name)` instance method. Document vendor-field mapping in docstrings. | L | Task 115 | done |
| 118 | Implement `SecurityFinding` model (`netex/src/models/security_finding.py`): severity (CRITICAL/HIGH/MEDIUM/LOW), category (one of the 7 plan review categories from PRD 5.2.1), description, recommendation, source_plugin, source_tool. | M | Task 115 | done |
| 119 | Implement Plugin Registry (`netex/src/registry/plugin_registry.py`): discover installed vendor plugins via Python entry points (D12, validated by Task 12 spike). Parse SKILL.md frontmatter, index tools by skill group. Methods: `list_plugins()`, `plugins_with_role(role)`, `plugins_with_skill(skill)`, `tools_for_skill(skill)`. | L | Task 115 | done |
| 120 | Implement Contract Validator (`netex/src/registry/contract_validator.py`): validate SKILL.md compliance with contract v1.0.0. Check required fields, valid skill group names, tool naming convention. | M | Task 119 | done |
| 121 | Write Vendor Plugin Contract spec (`contract/v1.0.0/`): `VENDOR_PLUGIN_CONTRACT.md`, `skill_groups.md`, `tool_naming.md`, `skill_md_reference.md`. | M | None | done |
| 122 | Design rollback workflow state machine (D15): implement `netex/src/workflows/workflow_state.py`. States: `planned -> executing -> step-N-complete -> failed-at-step-N -> rolling-back -> rollback-complete | rollback-failed`. Persist state to local JSON file per workflow run. Include: step log with timestamps, rollback actions per step (recorded at execution time), current state. This design is consumed by the Orchestrator in M4.3. | L | Task 115 | done |
| 123 | Create GitHub Actions CI for netex (`/.github/workflows/netex.yml`). | S | Task 115 | done |
| 124 | Write tests: abstract model serialization + vendor mapping, Plugin Registry discovery with mock SKILL.md files and entry points, contract validation (valid + invalid manifests), workflow state machine transitions + persistence. | L | Tasks 117-122 | done |
| 258 | Update Vendor Plugin Contract spec (`contract/v1.0.0/skill_groups.md`) to add `dns` role to the `netex_role` enum and add `profiles`, `analytics`, `logs`, `security-posture` to the skill group contract table. Update `VENDOR_PLUGIN_CONTRACT.md` with dns-role-specific sequencing rules (dns role executes after gateway in cross-vendor workflows -- gateway configures forwarders, dns plugin verifies resolution). | M | Task 121 | done |
| 259 | Extend abstract data model (`netex/src/models/abstract.py`): add `DNSProfile` (vendor-neutral representation of a DNS filtering profile -- name, security toggles, blocklists, deny/allow entries), `DNSForwarderMapping` (subnet/VLAN -> upstream DNS endpoint mapping), `DNSAnalyticsSummary` (query counts by status, top blocked). Add `from_vendor` / `to_vendor` support for `nextdns` vendor. | M | Task 117 | done |
| 260 | Update Plugin Registry to handle `dns` role in `plugins_with_role("dns")` queries. Verify nextdns SKILL.md is discovered and indexed correctly. Add `tools_for_skill("profiles")`, `tools_for_skill("analytics")`, `tools_for_skill("logs")` support. | S | Task 119 | done |

**Parallelizable:** Tasks 115, 121 can start immediately. Tasks 116-120, 122-123 depend on 115 and can run concurrently (7 tasks). Tasks 258-260 can run concurrently after their respective dependencies. Task 124 after 117-122.
**Milestone Value:** netex umbrella can discover installed vendor plugins (including nextdns), validate their contracts, provide a registry for skill-based tool lookup, and persist workflow state for rollback coordination. Abstract model enables vendor-neutral operations including DNS profiles.

### Milestone 4.2: OutageRiskAgent + NetworkSecurityAgent

| # | Task | Complexity | Dependencies | Status |
|---|------|-----------|--------------|--------|
| 125 | Implement OutageRiskAgent (`netex/src/agents/outage_risk_agent.py`): session path resolution (source IP -> traceroute -> switch port -> VLAN), risk tier classification (CRITICAL/HIGH/MEDIUM/LOW per PRD 10.3), single-pass batch assessment. Uses registry to find diagnostics/topology/clients tools across all installed plugins. **Session path resolution fallback:** (1) Check `OPERATOR_IP` env var, (2) inspect HTTP request headers if transport=http, (3) accept `--operator-ip` CLI override, (4) if none available, default to HIGH and state "session path could not be determined." | L | M4.1 | done |
| 126 | Implement NetworkSecurityAgent (`netex/src/agents/network_security_agent.py`) -- Role 1 (automatic plan review): 7 finding categories (PRD 5.2.1: VLAN isolation gap, overly broad rule, rule ordering risk, VPN split-tunnel, unencrypted VLAN, management exposure, DNS security). Read-only enforcement at tool registry level (agent can only call read tools). Output: list of `SecurityFinding` objects. If no issues: "Security review: no issues identified." | L | M4.1 | done |
| 127 | Implement NetworkSecurityAgent -- Role 2 (on-demand audit): 10 audit domains (PRD 5.2.2). Uses `registry.tools_for_skill("security")` and `registry.tools_for_skill("config")`. `--domain` filter. Output via OX formatters. Remediation commands always without `--apply`. | L | Task 126 | done |
| 128 | Write tests: OutageRiskAgent with mock plugin responses (test all 4 risk tiers, session path determination failure -> HIGH default, OPERATOR_IP env var, HTTP header fallback), NetworkSecurityAgent plan review (test all 7 finding categories), on-demand audit (test subset of domains). | L | Tasks 125-127 | done |
| 261 | Extend NetworkSecurityAgent finding categories to include DNS-layer findings when nextdns plugin is installed: (a) DNS security gaps -- profile missing threat protection toggles, (b) DNS filtering bypass -- VLAN without NextDNS profile forwarder (devices can use unfiltered DNS), (c) DNS logging gaps -- profile with logging disabled reduces audit trail. Uses `registry.plugins_with_role("dns")` to detect nextdns. Gracefully skips DNS checks when no dns-role plugin is installed. | M | Task 126 | done |
| 262 | Write tests for DNS-layer NetworkSecurityAgent findings: mock nextdns profile tools to return profiles with weak security settings, test finding generation for each DNS category, test graceful skip when no dns-role plugin installed. | M | Task 261 | done |

**Parallelizable:** Tasks 125 and 126 can run concurrently. Task 127 depends on 126. Task 261 depends on 126. Tasks 127 and 261 can run concurrently. Tasks 128 and 262 after their respective dependencies.
**Milestone Value:** Both safety agents operational. Every write plan gets an outage risk assessment and security review (including DNS-layer findings when nextdns installed) before presentation to the operator. On-demand security audit available.

### Milestone 4.3: Orchestrator + Core Cross-Vendor Commands

| # | Task | Complexity | Dependencies | Status |
|---|------|-----------|--------------|--------|
| 129 | Implement Orchestrator -- intent routing (`netex/src/agents/orchestrator.py`): single-vendor intent -> route directly to vendor plugin, cross-vendor intent -> umbrella workflow. Uses Plugin Registry to resolve vendor plugins by role. | M | M4.2 | done |
| 130 | Implement Orchestrator -- three-phase confirmation model: (a) Phase 1 "Gather & Resolve": identify ambiguities, batch AskUserQuestion using templates from Task 10 patterns, run OutageRiskAgent + NetworkSecurityAgent in parallel. (b) Phase 2 "Build & Present": construct ordered change plan, structure as `[OUTAGE RISK] -> [SECURITY] -> [CHANGE PLAN] -> [ROLLBACK]`, present via OX formatters. (c) Phase 3 "Single Confirmation": one AskUserQuestion with step count and system summary. | L | Task 129 | done |
...
| 132 | Implement `netex vlan configure` command/workflow: 7-step cross-vendor VLAN provisioning (PRD 5.3: gateway VLAN interface -> DHCP -> isolation rule -> reconfigure -> edge network -> port profiles -> SSID). Rollback on step 5+ failure. Uses workflow state machine. | L | Task 131 | done |
| 133 | Implement `netex vlan audit` command/workflow: compare VLANs across gateway and edge plugins. Surface: defined-in-gateway-only, defined-in-edge-only, mismatched subnets/DHCP. | M | Task 129 | done |
| 134 | Implement `netex topology` command: merge topology tools across all installed plugins into unified `NetworkTopology`. WAN -> gateway -> VLANs -> edge devices -> APs -> clients. | M | Task 129 | done |
| 135 | Implement `netex health` command: merge health tools across all plugins into unified severity-tiered report. Critical/High first regardless of source plugin. | M | Task 129 | done |
| 136 | Implement `netex firewall audit` command: cross-layer firewall analysis. Delegates consistency analysis to NetworkSecurityAgent. | M | Task 129 | done |
| 137 | Implement `netex secure audit` and `netex secure review` commands: delegate to NetworkSecurityAgent. Support `--domain` filtering. | M | M4.2 | done |
| 138 | Write tests for orchestrator routing, three-phase confirmation model, `vlan configure` full workflow (success + rollback scenarios + rollback failure), `vlan audit` with mismatched VLANs, cross-vendor topology merge. | L | Tasks 129-137 | done |
| 263 | Update `netex dns trace` command to include NextDNS profile resolution when nextdns plugin is installed: after tracing DNS query path through OPNsense Unbound forwarders, check if the upstream forwarder points to a NextDNS profile endpoint (match `dns.nextdns.io/{profile_id}` pattern). If so, query NextDNS analytics for that profile to confirm the domain was resolved/blocked and show the reason. Output: full DNS path trace from device -> VLAN -> OPNsense forwarder -> NextDNS profile -> resolution status. Gracefully degrade to existing behavior when nextdns plugin not installed. | L | Task 142, Plan Phase 3.5 | done |
| 264 | Implement `netex dns verify-profiles` command: cross-vendor VLAN-to-NextDNS-profile verification (Q14). For each VLAN: (a) check OPNsense Unbound DNS forwarder config for that subnet, (b) extract NextDNS profile ID from forwarder target URL, (c) query NextDNS analytics for that profile, (d) check source IPs in analytics match the VLAN subnet range, (e) report: VLAN name, expected profile, forwarder config status, analytics confirmation (queries seen from expected subnet Y/N). Surface mismatches: VLAN with no DNS forwarder, forwarder pointing to wrong profile, no analytics traffic from expected subnet. | L | Task 142, Plan Phase 3.5 | done |
| 265 | Implement `netex__dns__get_cross_profile_summary()` umbrella tool: query analytics status across all NextDNS profiles, return unified view (total queries, blocks, per-profile breakdown, encryption audit across all profiles). Uses `nextdns__profiles__list_profiles` + `nextdns__analytics__get_status` for each. | M | Plan Phase 3.5 | done |
| 266 | Write tests for `netex dns trace` with NextDNS integration (forwarder matches NextDNS pattern vs. non-NextDNS forwarder, nextdns plugin not installed), `dns verify-profiles` (matched VLAN-profile, mismatched, missing forwarder, no analytics traffic), and cross-profile analytics summary. | L | Tasks 263-265 | done |

**Parallelizable:** Task 129 first. Task 130 depends on 129. Task 131 depends on 130. Tasks 132-137 can run concurrently after 131 (6 tasks). Tasks 263-265 can run concurrently after their dependencies. Task 138 after 129-137. Task 266 after 263-265. Max 9 concurrent.
**Milestone Value:** Cross-vendor operations work including full DNS-layer awareness. Operators can provision VLANs across all three systems, audit consistency, view unified topology and health, run security audits, and verify end-to-end DNS filtering paths from VLAN to NextDNS profile.

### Milestone 4.4: Advanced Umbrella Commands + Docs

| # | Task | Complexity | Dependencies | Status |
|---|------|-----------|--------------|--------|
| 139 | Implement `netex network provision-site` command: parse YAML manifest (vlans, access_policy, wifi, port_profiles), single OutageRiskAgent + NSA pass for entire batch, dependency-ordered execution (gateway interfaces -> DHCP -> aliases -> rules -> edge networks -> WiFi -> port profiles), single confirmation. Uses workflow state machine for full-batch rollback. | L | M4.3 | done |
| 140 | Implement `netex verify-policy` command: run expected-allow and expected-block tests from manifest access_policy. Verify DHCP, DNS, WiFi SSID-to-VLAN mapping. Pass/fail per test. | L | Task 139 | done |
...
| 267 | Update `netex verify-policy` command to include DNS verification when nextdns plugin is installed: for each VLAN in the manifest, verify the VLAN's DNS forwarder points to the expected NextDNS profile (if `dns_profile` field present in manifest). Add `dns_profile` optional field to VLAN manifest schema. Report DNS verification results alongside existing DHCP, firewall, and WiFi verification. | M | Task 140, Plan Phase 3.5 | done |
| 268 | Update `netex network provision-site` command to support optional NextDNS profile linkage in VLAN manifest: if `dns_profile` is specified for a VLAN, verify the profile exists in NextDNS, then configure OPNsense Unbound DNS forwarder for that VLAN's subnet to point to the NextDNS profile endpoint (`dns.nextdns.io/{profile_id}`). Add as a step in the dependency-ordered execution (after gateway interfaces, before WiFi). | L | Task 139, Plan Phase 3.5 | done |
| 269 | Update `docs/netex/commands.md` to document nextdns-aware cross-vendor commands: `dns trace` with NextDNS resolution, `dns verify-profiles`, `verify-policy` DNS checks, `provision-site` DNS profile linkage. | M | Tasks 263-268 | done |
| 270 | Write 3 nextdns cross-vendor workflow examples: VLAN-to-profile pinning verification, DNS path troubleshooting (device -> VLAN -> forwarder -> NextDNS -> resolution), bulk profile security audit across the network. Follow 7-section template. | L | Tasks 263-268 | done |
| 271 | Write tests for `verify-policy` DNS checks and `provision-site` DNS profile linkage. Test with manifest that includes dns_profile (valid profile, invalid profile, nextdns plugin not installed). | M | Tasks 267-268 | done |

**Parallelizable:** Tasks 139, 141-143, 146-147 can run concurrently (6 tasks). Task 140 depends on 139. Tasks 144-145, 148-149 depend on 139-142. Tasks 267-268 depend on their respective tasks and Phase 3.5. Tasks 269-271 depend on 263-268. Max 10 concurrent.
**Milestone Value:** Complete netex umbrella with all PRD Section 5 commands plus full DNS-layer cross-vendor integration. All four plugins published to EmberAI marketplace. Operators can provision sites with end-to-end DNS filtering, verify VLAN-to-profile pinning, and trace DNS paths across the full stack.
...
### Milestone 4.7: Firewall CRUD + DNAT Port Forward Tools

Gaps discovered during live network configuration: the opnsense plugin can list firewall rules but cannot create, update, or delete them as individual operations. The matrix tool (`policy-from-matrix`) does not resolve OPNsense interface aliases. Port forward (DNAT) management has no tool coverage at all. API patterns for all operations were validated against a live OPNsense 26.x instance during the session that uncovered these gaps.

**Key API detail:** The firewall module uses `POST /api/firewall/filter/apply` (or `/api/firewall/d_nat/apply`) to push saved changes to the live pf ruleset. This is distinct from the `reconfigure` pattern used by other OPNsense modules. All write tools in this milestone must call `apply` after modifications.

| # | Task | Complexity | Dependencies | Status |
|---|------|-----------|--------------|--------|
| 185 | **Add `opnsense__firewall__create_rule` tool.** Low-level rule creation with full field control. API: `POST /api/firewall/filter/addRule` with `{"rule": {...}}` payload, then `POST /api/firewall/filter/apply`. Key fields: `enabled` (`"1"`/`"0"`), `action` (`"pass"`/`"block"`/`"reject"`), `quick` (`"1"` = first-match, `"0"` = last-match-wins), `interface` (OPNsense ID like `"lan"`, `"wan"`, `"opt3"`), `direction` (`"in"`/`"out"`), `ipprotocol` (`"inet"`/`"inet6"`/`"inet46"`), `protocol` (`"any"`, `"TCP"`, `"UDP"`, etc.), `source_net`/`destination_net` (CIDR, alias name, interface ID, or `"any"`), `sequence` (string number controlling evaluation order -- lower = first), `description`. Returns the new rule UUID. Write-gated (D8). | M | M4.5 (Task 171) | done |
| 186 | **Add `opnsense__firewall__update_rule` tool.** Update an existing rule by UUID. API: `POST /api/firewall/filter/set_rule/{uuid}` with `{"rule": {<partial fields>}}`, then `POST /api/firewall/filter/apply`. Accepts any combination of rule fields -- only sends fields being changed (partial update). **Validation note:** some field combinations have dependencies (e.g., changing `ipprotocol` to `"inet46"` fails if `destination_net` is an IPv4-only address). Tool should detect common conflicts and return a clear error with remediation guidance (e.g., "Create a separate IPv6 rule instead"). Tool signature: `opnsense__firewall__update_rule(uuid, *, action=None, interface=None, ipprotocol=None, source_net=None, destination_net=None, protocol=None, enabled=None, description=None, apply=False)`. Write-gated (D8). | M | M4.5 (Task 171) | done |
| 187 | **Add `opnsense__firewall__delete_rule` tool.** Delete a rule by UUID. API: `POST /api/firewall/filter/del_rule/{uuid}`, then `POST /api/firewall/filter/apply`. Return confirmation with deleted rule description for operator visibility. Write-gated (D8). | S | M4.5 (Task 171) | done |
| 188 | **Update `opnsense_firewall_policy_from_matrix` to resolve OPNsense interface aliases.** Currently the matrix tool only accepts CIDR addresses as source/destination. It should also accept OPNsense interface names (e.g., `"opt8"`, `"__opt8_network"`) and user-defined aliases (e.g., `"RFC1918"`, `"Firewall_Self"`). Before creating rules, resolve human-friendly names to OPNsense identifiers: look up interface aliases from `/api/firewall/alias/search_item` or the internal alias table. Support `"any"` as a valid source/destination. Error clearly when an alias cannot be resolved rather than passing invalid data to the API. | M | M4.5 (Task 171) | done |
| 189 | **Add `opnsense__firewall__list_port_forwards` tool.** List all DNAT port forward rules. API: `GET /api/firewall/d_nat/search_rule` with `rowCount=-1`. Return interface, protocol, source/destination with ports, target IP, target port, description, enabled status, and UUID for each rule. | M | M4.5 | done |
| 190 | **Add `opnsense__firewall__add_port_forward` tool.** Create a DNAT port forward rule. API: `POST /api/firewall/d_nat/addRule` with `{"rule": {...}}` payload, then `POST /api/firewall/d_nat/apply`. Key fields: `interface` (typically `"wan"`), `ipprotocol`, `protocol` (`"tcp"`, `"udp"`, `"tcp/udp"`), `source` (network, address, port, not), `destination` (network e.g. `"wanip"`, address, port, not), `target` (internal IP), `local-port` (internal port), `descr`, `pass` (`"pass"` to auto-create firewall allow rule), `log`. Returns the new rule UUID. Write-gated (D8). | M | M4.5 | done |
| 191 | **Add `opnsense__firewall__delete_port_forward` tool.** Delete a DNAT rule by UUID. API: `POST /api/firewall/d_nat/del_rule/{uuid}`, then `POST /api/firewall/d_nat/apply`. Write-gated (D8). | S | M4.5 | done |
| 192 | **Add firewall `apply` step to existing write tools.** The existing `opnsense_rule_create` command-level tool and `opnsense_firewall_policy_from_matrix` currently save rules but may not call `apply` to push them to the live pf ruleset. Audit both tools and add `POST /api/firewall/filter/apply` (or `/api/firewall/d_nat/apply`) after rule creation/modification. Ensure the apply step is covered by the write gate (only applied when `apply=True`). | S | M4.5 | done |
| 193 | **Write tests for all new firewall and DNAT tools.** Mock OPNsense API responses for: (a) filter rule CRUD (create, update partial fields, delete), (b) `apply` call verification after every modification, (c) write gate enforcement (env var disabled, `--apply` missing), (d) matrix tool with interface alias resolution (valid alias, unknown alias error), (e) DNAT port forward CRUD (list, add, delete), (f) validation error handling (e.g., IPv4-only destination with `inet46` protocol). Coverage in `opnsense/tests/test_firewall_crud.py` and `opnsense/tests/test_dnat.py`. | M | Tasks 185-192 | done |

**Parallelizable:** Tasks 185-187 (filter rule CRUD) can run concurrently. Task 188 (matrix alias support) is independent of 185-187 and can run in parallel. Tasks 189-191 (DNAT tools) can run concurrently with 185-188 (different API module). Task 192 (apply audit) is independent. Maximum 7 tasks concurrent. Task 193 (tests) depends on all others.
**Milestone Value:** Full conversational firewall management -- operators can create, update, and delete individual firewall rules and port forwards through natural language instead of running raw API calls. The matrix tool accepts interface names, not just CIDRs. All gaps discovered during live network configuration are closed.
