...
"""Structured error hierarchy for the opnsense plugin.
...
::
    ...
    NetexError (base)
    +-- AuthenticationError    # 401 -- missing or invalid API key/secret
...
class NetexError(Exception):
    ...
    endpoint:
        API endpoint path (e.g. ``/api/firewall/filter/searchRule``).
    retry_hint:
...
class WriteGateError(NetexError):
    ...
    The safety gate (see ``opnsense/safety.py``) enforces two preconditions
