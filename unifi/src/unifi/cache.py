...
carries its own TTL, enabling per-data-type expiration (e.g., device lists at
5 min, client lists at 30 sec). Concurrent requests for the same key are
coalesced via a single-flight pattern: only one fetch executes while other
callers await its result.

Typical usage with the UniFi API client::

    cache = TTLCache(max_size=500, default_ttl=300.0)

    devices = await cache.get_or_fetch(
        "devices:site-abc",
        fetcher=lambda: api.list_devices("site-abc"),
        ttl=300.0,
    )
...
class TTLCache:
    ...
    async def flush_by_prefix(self, prefix: str) -> None:
        ...
        ``await cache.flush_by_prefix("devices:")`` after a device update.
