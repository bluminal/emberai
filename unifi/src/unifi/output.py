...
All agents and commands in the unifi plugin use OX formatters to produce
...
def format_severity_report(title: str, findings: list[Finding]) -> str:
    ...
    Args:
        title: The report title (e.g., "Site Health Report").
...
def format_table(
    ...
    """Format a markdown table for device/client/interface inventories.
...
def format_key_value(
    ...
    detail views (device details, client info, interface config).
...
def format_risk_block(
    ...
    Args:
        ...
        affected_path: Optional network path at risk (e.g., "VLAN 10 -> switch port 3").
