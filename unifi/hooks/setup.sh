#!/usr/bin/env bash
# Post-install setup: creates venv and installs dependencies.
# Runs during plugin installation (Setup hook), NOT during MCP server startup.
...
if [ -d "$VENV_DIR" ]; then
    echo '{"systemMessage": "unifi plugin: dependencies already installed."}'
    exit 0
fi
